...
(function($) {
	...
	var tips = [], reBgImage = /^url\(["']?([^"'\)]*)["']?\);?$/i, rePNG = /\.png$/i, ie6 = $.browser.msie && $.browser.version == 6;
	...
	$.Poshytip.prototype = {
				...
				switch (this.opts.showOn) {
					case 'hover':
						...
						if (this.opts.allowTipHover) {
							this.$tip.bind({
								mouseenter : $.proxy(this.clearTimeouts, this),
		...
		mouseleave : function(delay, e) {
			if (!$.isNumeric(delay)) {
				e = delay;
				delay = this.opts.hideTimeout;
			}
			...
			this.clearTimeouts();
			this.hideTimeout = setTimeout($.proxy(this.hide, this), delay);
		},
		...
		show : function(timeOnScreen) {
			if (this.disabled || this.$tip.data('active'))
				return;

			if (!$.isNumber(timeOnScreen)) {
				timeOnScreen = this.opts.timeOnScreen;
			}
			...
			if (timeOnScreen) {
				this.autoHideTimer = setTimeout($.proxy(this.hide, this), timeOnScreen);

				if (this.opts.allowTipHover && timeOnScreen > 0) {
					this.$tip.one({
						mouseenter : $.proxy(this.clearTimeouts, this),
						mouseleave : $.proxy(this.mouseleave, this, timeOnScreen)
					});
				}
			}
		},
		hide : function() {
			if (this.disabled || !this.$tip || !this.$tip.data('active'))
				return;
		...
		bind : function(type, fn, data, context) {
			...
			this.$elm.bind(type + '.poshytip', data, $.proxy(fn, context));
		},
		// 取消绑定自定义事件
		unbind : function(type, fn) {
			this.$elm.unbind(type + '.poshytip', fn);
		},
		update : function(content, dontOverwriteOption) {
			...
			if (this.content !== newContent) {
				this.$inner.find('[evt]').unbind();
				this.$inner.empty().append(newContent);
				this.content = newContent;

				/* 处理提示内容中的自定义事件
				 * 格式：evt="触发事件名>自定义事件名"
				 * 如：<a evt="click>onConfirm" ...>确认</a>
				 */
				this.$inner.find('[evt]').each(function() {
					var $el = $(this), evt = $el.attr('evt').split('|');
					if (evt.length >= 2) {
						$el.bind(evt[0], function() {
							self.$elm.triggerHandler(evt[1] + '.poshytip', this)
						});
					}
				});
			}
		...
		calcPos : function() {
			...
			win, xL, xC, xR, yT, yC, yB;
			
			if(!this.opts.autoFlip){
				win = {
					l : 0,
					t : 0,
					w : 3000,
	...
	$.fn.poshytip = function(options) {
		...
		return this.each(function() {
			...
			var $el = $(this), inc = $el.data('poshytip');

			if (inc) {
				if (inc.$elm[0] === this && $.param(inc.userOpts) == $.param(opts)) {
					return true;
				} else {
					inc.destroy();
				}
	...
	$.fn.poshytip.defaults = {
		...
		autoFlip: true // 当tips超出显示区边缘时，是否自动翻转
	};
	...
	$.tips = {
		...
		_showMsg : function(el, cnt, css, delay, pos) {
			var $el;
			this.hide();
			$el = this.$el = $(el);
			...
			$el.poshytip('show', $.isNumber(delay) ? delay : 0);

			// delay 如果为函数对象{evtname:fn, evtname:fn...}, 视为自定义事件定义
			if ($.isObject(delay)) {
				$.each(delay, function(k, v) {
					if ($.isFunction(v)) {
						$el.poshytip('unbind', k);
						$el.poshytip('bind', k, v);
					}
				});
			}
		},

		info : function(el, cnt, delay, pos) {
			this._showMsg(el, cnt, 'tip-blue tip-info', delay, pos);
		},

		err : function(el, cnt, delay, pos) {
			this._showMsg(el, cnt, 'tip-red tip-err', delay, pos);
		},

		warn : function(el, cnt, delay, pos) {
			this._showMsg(el, cnt, 'tip-yellow tip-warn', delay, pos);
		},

		success : function(el, cnt, delay, pos) {
			this._showMsg(el, cnt, 'tip-green tip-success', delay, pos);
		},

		hide : function() {
			if (this.$el == null)
				return;
			this.$el.poshytip('hide');
			delete this.$el;
		}
	};
